            ...
            <div style={AppStyles.window}>
                <div>
                    ...
                    <div>
                        <div>
                            <h5>Coin Type</h5>
                            <select style={{width:'5em'}} defaultValue={null} 
                                    value={this.state.payCoin.name} 
                                    onChange={(e)=>this.setState({...this.state, payCoin: this.props.ownedCoins.find(coin=>coin.name===e.currentTarget.value) })}>
                    ...
                    <div>
                        <div>
                            <h5>Coin Type</h5>
                            <select style={{width:'5em'}} defaultValue={this.props.coin.name} 
                                    value={this.state.payCoin.name} 
                                    onChange={(e)=>this.setState({...this.state, payCoin: this.props.ownedCoins.find(coin=>coin.name===e.currentTarget.value) })}>
                                <option value={null}>Select...</option>
                                {this.props.ownedCoins.map(coin=><option value={coin.name}>{coin.name}</option>)}
                        ...
                        <div style={{display:'flex'}}>
                            ...
                            <div style={{width:'4em'}}>{this.props.coin.value}</div>
                            <div style={{width:'4em'}}>{this.props.coin.difficulty}</div>
                            <div>{this.state.amount*this.props.coin.value} (+{this.props.coin.difficulty} exchange fee)</div>
                        </div>
                    </div>
                    <div style={{display:'flex', justifyContent:'flex-end', alignItems:'center'}}>
                        {LightButton(true, this.props.hide, 'Cancel')}
                        {Button(true, ()=>{onConvertCoin(this.props.me, this.state.payCoin, this.state.amount, this.props.coin); this.props.hide()}, 'Ok')}
                    </div>
                </div>
         </div>
        )
